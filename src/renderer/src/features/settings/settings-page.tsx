...
import { Settings as SettingsIcon, Palette, Cpu, Puzzle, Zap, MessageSquareText, Mic,
  Layers, ChevronLeft, LayoutPanelLeft, Boxes, BookOpenText, type LucideIcon
} from 'lucide-react'
...
import { KnowledgeBaseSettings } from '@renderer/features/settings/knowledge-base-settings'

type SettingsPage = 'general' | 'appearance' | 'rightPanels' | 'pi' | 'models' | 'knowledge' | 'skills' | 'prompts' | 'extensions' | 'adapters' | 'voice'
...
export function SettingsPage() {
  ...
  const PAGES: { key: SettingsPage; icon: LucideIcon; label: string }[] = [
    ...
    { key: 'knowledge', icon: BookOpenText, label: t('settings:nav.knowledge') },
  ...
  const widePages: SettingsPage[] = ['rightPanels', 'pi', 'models', 'knowledge', 'skills', 'prompts', 'extensions', 'adapters', 'voice']
    ...
    <SettingsDraftProvider>
      <div className="flex h-full min-h-0 w-full overflow-hidden">
        ...
        <SettingsMain wide={wide} footer={<SettingsSaveBar />}>
          ...
          {page === 'knowledge' && <KnowledgeBaseSettings />}
