...
export function MainLayoutShell({
  ...
  const filesChatPreview =
    rightVisible && activePanel === 'files' && filesPreviewChatExpand && !rightCollapsed
  ...
  const rightCol = !rightVisible
    ? '0px'
    : rightCollapsed
      ? `${RIGHT_COLLAPSED_RAIL_PX}px`
      : `${rightWidth}px`
    ...
    <div
      className={cn(
        ...
        rightDragging && 'shell-right-dragging',
          rightVisible && rightCollapsed && !filesChatPreview && 'shell-right-rail-only',
      )}
    ...
    >
      ...
      <div
        className={cn(
          ...
          rightVisible && rightCollapsed && 'shell-track-right-rail',
        )}
        style={{ background: 'var(--bg-base)' }}
        aria-hidden={!rightVisible}
      >
        {rightVisible && !rightCollapsed && !filesChatPreview ? (
          <PanelResizeEdge
            side="right"
