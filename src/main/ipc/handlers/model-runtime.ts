...
export function registerModelRuntimeHandlers(): void {
  ...
  registerHandler('ipc:model.set', async (req) => {
    ...
    if (!workerManager.isRunning) {
      ...
      if (cwd && !isSandboxWorkspacePath(cwd)) {
        await workerManager.start(cwd)
      } else {
        const { SettingsManager, getAgentDir } = await getActiveSdkModule()
        const settings = SettingsManager.create(cwd || process.cwd(), getAgentDir())
        settings.setDefaultModelAndProvider(provider, modelId)
        await settings.flush()
        return { modelId: `${provider}/${modelId}`, persistedDefault: true }
      }
    }
