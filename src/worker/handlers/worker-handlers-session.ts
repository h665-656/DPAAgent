...
export async function handleSetmodel(msg: WorkerIncomingMessage, reply: WorkerReply): Promise<void> {
        ...
        try {
          // Resolve from the session registry; pi-ai is a nested dependency and is not hoisted.
          ...
          if (!model) {
            reply({ type: 'error', error: `Model not found: ${provider}/${modelId}` })
            return
          }
          ...
          reply({ type: 'error', error: `setModel failed: ${errorMessage(e)}` })
          return
        }
